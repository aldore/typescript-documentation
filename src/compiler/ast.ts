...
module TypeScript {
	...
	export class AST extends ASTSpan {
		...
		public emit(emitter: Emitter, tokenId: TokenID, startLine: bool) {
			...
			switch (this.nodeType) {
				case NodeType.This:
					//FIXME: this way to inlining getter so wrong :( 
	...
	export class BinaryExpression extends Expression {
		...
		public emit(emitter: Emitter, tokenId: TokenID, startLine: bool) {
			...
			if (this.nodeType != NodeType.Comma && binTokenId != undefined) {
				...
				var pGetterSymbol: FieldSymbol = null;
				var pGetter: FuncDecl = null;
				var isSetter: bool = false;

				var op1: Identifier = <Identifier>this.operand1;
				if (this.operand1.nodeType === NodeType.Dot &&
					(<BinaryExpression>this.operand1).operand2.nodeType == NodeType.Name) {
					var id: Identifier = <Identifier>(<BinaryExpression>this.operand1).operand2;
					var target: AST = (<BinaryExpression>this.operand1);
					//only functions with resolved symbols can be inlined
					if (isDefAndNotNull(id.sym) && id.sym.isAccessor()) {
						isSetter = binTokenId === TokenID.Equals;
						
						var sym: TypeSymbol = isSetter? (<FieldSymbol>id.sym).setter: (<FieldSymbol>id.sym).getter;
						var funcDecl: FuncDecl = sym ? <FuncDecl>(sym.declAST) : null;

						//block getter printing
						pGetterSymbol = <FieldSymbol>id.sym;
						pGetter = <FuncDecl>pGetterSymbol.declAST;
						pGetterSymbol.declAST = null;

						if (!isNull(funcDecl) &&
							funcDecl.isInline() &&
							emitter.inlineEngine.inlineFunction(emitter, target, funcDecl, this.operand2)) {
				...
				switch (this.nodeType) {
					case NodeType.Dot:
						if (!emitter.tryEmitConstant(this)) {
							...
							var id: Identifier = <Identifier>this.operand2;
							//only functions with resolved symbols can be inlined
							if (isDefAndNotNull(id.sym) && id.sym.isAccessor()) {
								var funcDecl: FuncDecl = <FuncDecl>id.sym.declAST;
								
								if (!isNull(funcDecl) &&
									funcDecl.isInline() &&
									emitter.inlineEngine.inlineFunction(emitter, this, funcDecl)) {
