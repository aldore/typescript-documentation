...
module TypeScript {
    ...
    export class Emitter {
        ...
        //inline backend
        public inlineEngine = new InlineEngine;
            ...
            if (!this.emitSuperCall(callNode)) {
                if (!hasFlag(callNode.flags, ASTFlags.ClassBaseConstructorCall)) {
                    ...
                    if (callNode.target.nodeType == NodeType.Super && this.emitState.container == EmitContainer.Constructor) {
                        ...
                        //(target.nodeType == NodeType.Name || target.nodeType == NodeType.Dot) && 
                        if (isDefAndNotNull(callNode.signature)) {
                            //trying to inline simple calls
                            funcDecl = callNode.signature.declAST;

                            if (isDefAndNotNull(funcDecl) && funcDecl.isInline()) {
                                //we know type, and it inline               
                            ...
                            if (hasFlag(fieldSym.flags, SymbolFlags.ModuleMember)) {
                                if ((sym.container != this.checker.gloMod) && ((hasFlag(sym.flags, SymbolFlags.Property)) || hasFlag(sym.flags, SymbolFlags.Exported))) {
                                    if (hasFlag(sym.declModule.modFlags, ModuleFlags.IsDynamic)) {
                                    	...
                                    	var contName: string = this.inlineEngine.normalizeModuleName(this, sym.container);
