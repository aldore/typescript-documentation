...
///<reference path='InlineEngine.ts' />
...
module TypeScript {
    ...
    export class TypeScriptCompiler {
        ...
        public emitDocumentation() {
            ...
            var documentationEmitter: DocumentationEmitter = null;

            for (var i = 0, len = this.scripts.members.length; i < len; i++) {
                var script = <Script>this.scripts.members[i];
                if (this.emitSettings.outputMany || documentationEmitter === null) {
                    documentationEmitter = this.emitDocumentationUnit(script, !this.emitSettings.outputMany);
                } else {
                    // Emit in existing emitter
                    this.emitDocumentationUnit(script, true, documentationEmitter);
                }
            }

            if (documentationEmitter) {
                documentationEmitter.Close();
            }
        }

        private emitDocumentationUnit(script: Script, reuseEmitter?: bool, documentationEmitter?: DocumentationEmitter) {
            ...
            if (!documentationEmitter) {
                ...
                documentationEmitter.setDeclarationFile(documentationFile);
            }

            documentationEmitter.emitDeclarations(script);
